...
- Пользовательские тексты, help, status/info и operator-facing сообщения должны проходить через `app/i18n/`.
- Для стабильных ключевых сообщений используй `app/i18n/messages.py`, для локализуемых low-level строк допустим `tr(...)` из `app/i18n/locale.py`.
...
- Если изменение логически тянет на релизный bump, обновляй версию через `uv version --bump patch|minor|major` по semver и добавляй соответствующий Git tag.
...
- `app/i18n/` — локализация интерфейса и operator-facing сообщений
