...
from app.interfaces.menu import (
    bbr,
    docker,
    fail2ban,
    run,
...
def test_main_menu_displays_inline_statuses():
    ...
    with (
        patch(
            "app.interfaces.menu.run.build_main_menu_items",
...
def test_bbr_interactive_status_action_prints_service_status():
    ...
    with (
        patch("app.interfaces.menu.bbr.BBRService", return_value=service),
...
def test_docker_interactive_status_action_prints_service_status():
    ...
    with (
        patch("app.interfaces.menu.docker.DockerService", return_value=service),
...
def test_fail2ban_interactive_status_action_prints_service_status():
    ...
    with (
        patch("app.interfaces.menu.fail2ban.Fail2BanService", return_value=service),
        patch("builtins.input", side_effect=["5", "0"]),
        patch("builtins.print") as mock_print,
    ):
...
def test_uv_interactive_status_action_prints_service_status():
    ...
    with (
        patch("app.interfaces.menu.uv.UVService", return_value=service),
        patch("builtins.input", side_effect=["3", "0"]),
        patch("builtins.print") as mock_print,
    ):
        uv.interactive_run()
